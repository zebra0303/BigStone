import { addMonths, setDate, isPast, getDay, addDays, startOfDay } from "date-fns";
...
export function getNextValidDueDate(
    ...
    if (recurring.type === "WEEKLY" && recurring.weeklyDays && recurring.weeklyDays.length > 0) {
    ...
    // Fallback for NTH or YEARLY which are more complex, just align to the base date and let user manually pick
    return baseDate;
}
