...
function groupTodosByMonth(todos: Todo[]) {
    ...
    todos.forEach((todo) => {
        ...
        const dateObj = todo.completedAt ? new Date(todo.completedAt) : new Date(todo.dueDate);
...
export function ArchivePage() {
    ...
    const archiveTodos = useMemo(() => {
        ...
        return todos
            ...
            .sort((a, b) => {
                const dateA = a.completedAt ? new Date(a.completedAt).getTime() : new Date(a.dueDate).getTime();
                const dateB = b.completedAt ? new Date(b.completedAt).getTime() : new Date(b.dueDate).getTime();
                return dateB - dateA; // Descending (newest first)
            });
    }, [todos]);

    const groupedTodos = useMemo(() => groupTodosByMonth(archiveTodos), [archiveTodos]);
    const monthKeys = Object.keys(groupedTodos).sort((a, b) => b.localeCompare(a)); // Reverse alphabetical for YYYY년 MM월
        ...
        <div className="mx-auto max-w-4xl p-4 md:p-8 space-y-8">
            <header className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-200 pb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">완료된 지난 목표들을 여기서 확인하세요.</p>
                    </div>
                </div>

                <Button variant="outline" onClick={() => navigate("/")} className="shrink-0 flex items-center gap-2">
            ...
            {monthKeys.length === 0 ? (
                <div className="text-center py-20 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                    <p className="text-gray-500 text-lg">아직 보관된 완료 일정이 없습니다.</p>
                </div>
            ) : (
                    ...
                    {monthKeys.map((month) => (
                        <section key={month} className="space-y-4">
                            ...
                            <div className="bg-white p-4 md:p-6 rounded-xl border border-gray-100 shadow-sm">
                                <TodoList todos={groupedTodos[month]} emptyMessage="기록이 없습니다." />
                            </div>
                        </section>
                    ))}
                </div>
